...
class TPN_attention(nn.Module):
    ...
    def __init__(self,
        ...
        super(TPN_attention, self).__init__()
    ...
    def forward(self, inputs, target=None):
        ...
        # x = (torch.rand([5, 2048, 8, 4, 4]), torch.rand([5, 2048, 8, 4, 4]), torch.rand([5, 2048, 8, 4, 4]))
        # bs, ch, d, hei, wei = outs[0].size()
        # #### triple attention
        # conncat_tensor_01 = torch.cat([outs[0], outs[1]], dim=1)
        # alpha_01 = F.softmax(conncat_tensor_01, dim=1)
        #
        # conncat_tensor_20 = torch.cat([outs[2], outs[0]], dim=1)
        # alpha_20 = F.softmax(conncat_tensor_20, dim=1)
        #
        # conncat_tensor_12 = torch.cat([outs[1], outs[2]], dim=1)
        # alpha_12 = F.softmax(conncat_tensor_12, dim=1)
        #
        # alphaD_0 = alpha_01[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        # alphaR_0 = alpha_01[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])
        #
        # alphaD_1 = alpha_20[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        # alphaR_1 = alpha_20[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])
        #
        # alphaD_2 = alpha_12[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        # alphaR_2 = alpha_12[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])
        #
        # temp_feature_0 = self.nonblock(outs[0], outs[1], alphaR_0, False)
        # temp_feature_1 = self.nonblock(outs[1], outs[2], alphaR_2, False)
        # temp_feature_2 = self.nonblock(outs[2], outs[0], alphaR_1, False)
        #
        # outs = (temp_feature_0,temp_feature_1,temp_feature_2)
        ...
        alpha_23 = F.softmax(conncat_tensor_23, dim=1)

        conncat_tensor_30 = torch.cat([outs[3], outs[0]], dim=1)
        alpha_30 = F.softmax(conncat_tensor_30, dim=1)

        alphaD_0 = alpha_01[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        alphaR_0 = alpha_01[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])

        alphaD_1 = alpha_12[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        alphaR_1 = alpha_12[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])

        alphaD_2 = alpha_23[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        alphaR_2 = alpha_23[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])

        alphaD_3 = alpha_30[:, 0, :, :, :].reshape([bs, d * hei * wei, 1])
        alphaR_3 = alpha_30[:, 1, :, :, :].reshape([bs, d * hei * wei, 1])

        temp_feature_0 = self.nonblock(outs[0], outs[1], alphaR_0, False)
        temp_feature_1 = self.nonblock(outs[1], outs[2], alphaR_1, False)
        temp_feature_2 = self.nonblock(outs[2], outs[3], alphaR_2, False)
        temp_feature_3 = self.nonblock(outs[3], outs[0], alphaR_3, False)

        outs = [temp_feature_0, temp_feature_1, temp_feature_2, temp_feature_3]
