...
### txt 数据读取 用作后续的数据集准备
file_path = '/hy-tmp/Feature_fusion_au_mta_save/' ## 由MTA_feature_extraction_AU得到


train_file_index = '../train_valid_index.npy' ## 训练对应的索引
test_file_index = '../test_index.npy' ## 测试对应的索引
...
#     print(f'predict result:{len(full_video_predicts_list)}')
#     print(f'predict result 的 std :{torch.std(full_video_predicts_th).data}')
    
#     print(f'ground truth:{len(full_video_groud_truth_list)}')
...
for epoch in range(epochs):
    ...
    if test_rmse < 6.80 and test_ccc > 0.15:
        torch.save(model.state_dict(), os.path.join(weights_dir,
                                        f'Epcoh:{str(epoch)}_Rmse:{str(test_rmse)}_PCC:{str(test_pcc)}_CCC:{str(test_ccc)}.pth'))
