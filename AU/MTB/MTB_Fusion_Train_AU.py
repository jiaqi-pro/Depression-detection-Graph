...
### vgg使用
train_file = '../train_valid_au.txt'
test_file = '../test_au.txt'
...
# in_channel = 4096
# input_channels = [256, 512, 1024, 4096]
# attention_channels = 2048
# outchannels = 512
# model = MTB(in_channel=in_channel, input_channels=input_channels, attention_channels=attention_channels,
#             outchannels=outchannels)
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for train_data in tqdm(train_loader):
        ...
        train_feature = train_feature.float().to(device)
    ...
    print(f'Epoch: {epoch} 的 train loss: {train_loss_avg}')
    return model
...
def evaliation(epoch, model, device, loader, file_count):
    ...
    for valid_data in tqdm(loader):
        ...
        valid_feature = valid_feature.float().to(device)
...
# model.load_state_dict(torch.load("/hy-tmp/Code/MTB_results/2022-08-21-11:18:13/weights/Epcoh:46_Rmse:6.845022201538086_PCC:0.2333487301766989_CCC:0.13534336370107866.pth"))  #model.load_state_dict()函数把加载的权重复制到模型的权重中去
...
for epoch in range(epochs):
    ...
    if test_rmse < 7.69 and test_ccc > 0.108:
        torch.save(model.state_dict(), os.path.join(weights_dir,
                                        f'Epcoh:{str(epoch)}_Rmse:{str(test_rmse)}_PCC:{str(test_pcc)}_CCC:{str(test_ccc)}.pth'))
